...
test.each([
	...
	'expects match funding of %sp for %sp donation, %s% rate, %sp remaining, %sp limit, %sp already',

	async (expected, donationAmount, matchFundingRate, matchFundingRemaining, matchFundingPerDonationLimit, alreadyMatchFunded) => {
		expect(matchFunding({
			donationAmount,
			alreadyMatchFunded,
			matchFundingRate,
