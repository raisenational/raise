...
import {Form, type FormProps, type InputType} from './Form';
...
const PropertyEditor = <I extends Record<string, any>>({
	...
	if (!item || ('loading' in item && item.loading)) {
		return <div className='overflow-x-auto bg-black bg-opacity-20 rounded p-4'><span className='animate-pulse'>Loading...</span></div>;
	}

	if ('loading' in item && item.error && item.error instanceof Error) {
		return <Alert variant='error'>{item.error}</Alert>;
	}

	if ('loading' in item && item.data === undefined) {
		return <Alert variant='error'>Data not found</Alert>;
	} // NB: A common cause of this is trying to find a new item you just created without refreshing the data (try useAxios.clearCache()).
	...
	return (
		<>
			<Modal open={editingProperty !== undefined} onClose={() => {
				setEditingProperty(undefined);
			}}>
				{editingProperty && (
					<Form<Partial<I>>
						title={`Editing ${definition[editingProperty]?.label?.toLowerCase()}`}
						definition={{
							[editingProperty]: {
			...
			<Table
				...
				renderItem={(i) => (
					<tr key={String(i.property)} className={classNames('hover:bg-black hover:bg-opacity-20', {'cursor-pointer': definition[i.property]?.inputType !== undefined})} onClick={definition[i.property]?.inputType === undefined
						? undefined
						: (e) => {
							tableOnClick(i, e);
						}}>
