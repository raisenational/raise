import {
	FormProvider, type SubmitHandler, useForm, type UseFormReturn, useWatch,
...
const DonationPageV2: React.FC<Props> = ({
	...
	return (
		<>
			...
			<Section>
				{fundraiser.error && !fundraiser.loading
...
type IntroFundraiserProps = {
	title: string;
	charities: CharityDefinition[];
	fundraiser: ResponseValues<PublicFundraiser, unknown>;
	openModal: () => void;
	reflectionFormHref: string;
};
					...
					'The charities': (
						<div className='space-y-8'>
							...
							{charities.map((charity) => (
								<div key={charity.name} className='space-y-2'>
									{charity.description}
...
const Tabs: React.FC<{tabs: Record<string, React.ReactNode>}> = ({tabs}) => {
	...
	return (
		<div className='bg-white text-black rounded shadow-raise overflow-hidden'>
			<div className='flex'>
				{ }
				{keys.map((k, i) => <Link key={k} onClick={() => {
					setOpen(k);
				}} className={classNames('flex-1 border-gray-300 py-2', {'border-l-2': i !== 0, 'border-b-2 bg-gray-200': k !== open})}>{k}</Link>)}
			</div>
		...
		<div className='space-y-4'>
			...
			<div className='flex flex-col gap-4'>
				{charities.map((charity) => {
					...
					return (
						<Button key={charity.name} variant={selected ? 'purple' : 'gray'} onClick={() => {
							setValue('charity', charity.name);
							void trigger('charity');
						}} skew={false} className={classNames('py-2 px-3 text-left justify-center items-center ml-0')}>
	...
	return (
		<>
			...
			<div className='mt-2 grid grid-cols-2 gap-4'>
				<Button variant={watches.recurrenceFrequency === 'ONE_OFF' ? 'purple' : 'gray'} onClick={() => {
					setValue('donationAmount', format.amountShort(fundraiser.currency, fundraiser.suggestedDonationAmountOneOff, false));
					setValue('recurrenceFrequency', 'ONE_OFF');
					void trigger();
				}} skew={false} className={classNames('p-2 text-center flex justify-center items-center', {'text-gray-200': watches.recurrenceFrequency !== 'ONE_OFF'})}>
					a one-off donation
				...
				<Button variant={watches.recurrenceFrequency === 'WEEKLY' ? 'purple' : 'gray'} onClick={() => {
					setValue('donationAmount', format.amountShort(fundraiser.currency, fundraiser.suggestedDonationAmountWeekly, false));
					setValue('recurrenceFrequency', 'WEEKLY');
					void trigger();
				}} skew={false} className={classNames('p-2 text-center ml-0 flex justify-center items-center', {'text-gray-200': watches.recurrenceFrequency !== 'WEEKLY'})}>
		...
		<>
			...
			<div>
				<LabelledInput
					...
					{...register('donorEmail', {
						validate(s) {
							if (!s) {
								return 'We need your email to send you an invite, and to identify your donation if you contact us.';
							}
			...
			{fundraiser.suggestedContributionAmount !== null && (
				<>
					...
					<div className='mt-2 grid grid-cols-2 gap-4'>
						<Button variant={contributionAmount > 0 ? 'purple' : 'gray'} onClick={() => {
							setValue('contributionAmount', format.amountShort(fundraiser.currency, fundraiser.suggestedContributionAmount ?? 10_00, false));
							void trigger('contributionAmount');
						}} skew={false} className={classNames('p-2 text-center  flex justify-center items-center', {'text-gray-200': contributionAmount <= 0})}>
							I want to contribute
						</Button>
						<Button variant={contributionAmount <= 0 ? 'purple' : 'gray'} onClick={() => {
							setValue('contributionAmount', '0');
							void trigger('contributionAmount');
						}} skew={false} className={classNames('p-2 text-center ml-0 flex justify-center items-center', {'text-gray-200': contributionAmount > 0})}>
...
const DonationFormDisplay: React.FC<{formMethods: UseFormReturn<DonationFormResponses>; watches: DonationFormResponses; fundraiser: PublicFundraiser}> = ({formMethods: {register}, watches, fundraiser}) => (
	<>
		...
		<DonationCard
			...
			matchFundingAmount={watches.donationAmountHidden
				? undefined
				: calcMatchFunding({
					donationAmount: parseMoney(watches.donationAmount),
...
const DonationFormPayment: React.FC<{formMethods: UseFormReturn<DonationFormResponses>; watches: DonationFormResponses; fundraiser: PublicFundraiser; setPayButton: (e: JSX.Element) => void; setPiResponse: (piResponse: PublicPaymentIntentResponse) => void; onPaymentSuccess: () => void}> = ({
	...
	useEffect(() => {
		...
		fetchPiResponse({fundraiserId: fundraiser.id}, data).then((r) => {
			setPiResponse(r.data);
		}).catch(() => {/* noop, handled by useReq hook */});
