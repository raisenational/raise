...
export const LabelledInput = React.forwardRef<HTMLInputElement, LabelledInputProps>(({
  ...
  return (
    <div className="border-4 border-white p-2 relative mb-4 rounded">
      {label && <label htmlFor={id} className={classNames('absolute bg-raise-blue -top-3 left-8 px-2 font-black text-xl leading-none', { 'text-raise-red': error })}>{label}</label>}
      <div className="flex flex-row mb-1">
        ...
        <input
          ...
          required
          className="bg-transparent w-full h-full resize-none border-none focus:bg-black focus:bg-opacity-10 p-2 rounded focus:outline-none"
          {...rest}
        />
        {suffix && (
...
export const SignUpForm = <T extends FieldValues>({
  title, warning, definition, initialValues, subscribe, onSubmit,
}: FormProps<T>) => {
  ...
  const promiseFail = (err: any) => {
    if (err instanceof Error) setError(err);
    else setError(new Error(String(err)));
  };
  const promiseSuccessful = () => {
    setSuccess(true);
  };
  ...
  const internalOnSubmit: SubmitHandler<T> = () => {
    ...
    try {
      ...
      if (promise) {
        return promise.then(promiseSuccessful, promiseFail);
      }
    } catch (err) {
      promiseFail(err);
    }
  ...
  return (
    <FormProvider {...formMethods}>
      <form onSubmit={handleSubmit(internalOnSubmit)} className="max-w-lg mx-auto">
        ...
        {success && <Alert className="mt-2" variant="success">{subscribe ? 'You have successfully signed up!' : 'You have successfully unsubscribed!'}</Alert>}
        <Button variant="red" className="mt-4 " onClick={handleSubmit(internalOnSubmit)} disabled={isSubmitting}>{isSubmitting ? 'Submitting...' : 'Submit'}</Button>
      </form>
    </FormProvider>
  );
};
