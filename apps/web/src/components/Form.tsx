...
import {
	...
	type FieldValues,
	FormProvider,
	type Path, type SubmitHandler, useForm, useFormContext, useWatch,
...
export const toInput = <T,>(raw: T, inputType: InputType<T>): string | string[] | boolean => {
	if (raw !== undefined && inputType === 'hidden') {
	...
	if (inputType === 'amount') {
		return (raw as unknown as number / 100).toFixed(2);
	}

	if (inputType === 'date' || inputType === 'datetime-local') {
		return new Date((raw as unknown as number * 1000) - (new Date(raw as unknown as number * 1000).getTimezoneOffset() * 60000)).toISOString().slice(0, 19);
	}
...
export const fromInput = <T,>(raw: string | boolean, inputType: InputType<T>, selectOptions: T extends string[] ? (string[] | Record<string, string>) : undefined): T => {
	if (inputType === 'hidden') {
	...
	if (inputType === 'text' || inputType === 'tel' || inputType === 'email') {
		return (raw === '' ? null : raw) as unknown as T;
	}

	if (inputType === 'checkbox' || typeof raw === 'boolean') {
		return raw as unknown as T;
	} // NB: typeof raw === "boolean" if-and-only-if inputType === "checkbox"

	if (inputType === 'number') {
		return ifNaN(parseInt(raw, 10), null) as unknown as T;
	}

	if (inputType === 'date' || inputType === 'datetime-local') {
		return ifNaN((new Date(raw).getTime()) / 1000, null) as unknown as T;
	}

	if (inputType === 'amount') {
		return ifNaN(Math.round(parseFloat(raw) * 100), null) as unknown as T;
	}

	if (inputType === 'select' && selectOptions) {
		return (Array.isArray(selectOptions) ? (selectOptions.includes(raw) ? raw : null) : (selectOptions[raw] ? raw : null)) as unknown as T;
	} // NB: selectOptions !== undefined if typeof raw === "select"

	if (inputType === 'multiselect' && selectOptions) {
		return (raw as unknown as string[]).filter((r) => (Array.isArray(selectOptions) ? (selectOptions.includes(r) ? r : null) : (selectOptions[r] ? r : null))) as unknown as T;
	} // NB: selectOptions !== undefined if typeof raw === "multiselect"
...
export const LabelledInput = React.forwardRef<HTMLInputElement, LabelledInputProps>(({
	...
	if (type === 'hidden') {
		return <input id={id} ref={ref} type={type} className={className} {...rest} />;
	}

	if (type === 'select' || type === 'multiselect') {
		// disable eslint as we assume these don't ever change per render
		...
		if (!formContext) {
			...
			return (
				<div className={className}>
					...
					{/* eslint-disable-next-line @typescript-eslint/ban-ts-comment */}
					{/* @ts-ignore */}
					<Select type={type} value={value} onChange={(v) => {
						setValue(v);
						if (rest.onChange) {
							rest.onChange({target: {value: v}} as React.ChangeEvent<HTMLInputElement>);
						}
			...
			<Controller
				...
				render={({field}) => (
					<div className={className}>
						...
						{/* eslint-disable-next-line @typescript-eslint/ban-ts-comment */}
						{/* @ts-ignore */}
						<Select type={type} value={field.value} onChange={(v) => {
							field.onChange({target: {value: v}} as React.ChangeEvent<HTMLInputElement>);
						}} error={error} options={options} />
						{error && <span className='text-raise-red'>{error}</span>}
					</div>
				)}
...
const Select: React.FC<({type: 'select'; value?: string; onChange: (s: string) => void} | {type: 'multiselect'; value?: string[]; onChange: (s: string[]) => void}) & {options: string[] | Record<string, string | null>; error?: string}> = (props) => {
	...
	const options: Record<string, string | null> = Array.isArray(props.options)
		? props.options.reduce<Record<string, string>>((acc, cur) => {
			acc[cur] = cur;
			return acc;
		}, {})
		: (props.options);
...
type PropertyDefinition<I, V> = {
	...
	| {inputType: InputType<V> & ('select' | 'multiselect'); selectOptions: readonly string[] | Record<string, string>}
);
...
export const Form = <T extends FieldValues>({
	...
	const internalOnSubmit: SubmitHandler<T> = async () => {
		try {
			...
			<form onSubmit={handleSubmit(internalOnSubmit)}>
				...
				{Object.entries(definition).map(([_k, _v], i, arr) => {
					...
					if (nInputType === 'hidden') {
						return <input key={k} type='hidden' {...register(k)} />;
					}
