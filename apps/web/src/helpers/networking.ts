...
import _axios, {
	type AxiosError, type AxiosInstance, type AxiosRequestConfig, type AxiosResponse,
} from 'axios';
import {setupCache} from 'axios-cache-interceptor';
import {useEffect, useState} from 'react';
import env from '../env/env';
import {makeClient, routes, type Routes} from './generated-api-client';
...
const getAuthFromLocalStorage = (): AuthState | undefined => {
	...
	if (value !== null) {
		try {
			...
			if (typeof parsed.accessToken !== 'object') {
				return undefined;
			}

			if (typeof parsed.accessToken.value !== 'string') {
				return undefined;
			}

			if (typeof parsed.accessToken.expiresAt !== 'number') {
				return undefined;
			}

			if (typeof parsed.refreshToken !== 'object') {
				return undefined;
			}

			if (typeof parsed.refreshToken.value !== 'string') {
				return undefined;
			}

			if (typeof parsed.refreshToken.expiresAt !== 'number') {
				return undefined;
			}

			if (parsed.refreshToken.expiresAt < (Date.now() / 1000)) {
				return undefined;
			}
			...
			if (!parsed.groups.every((g: unknown) => typeof g === 'string')) {
				return undefined;
			}
...
const logoutOnTokenExpiry = async (err: unknown) => {
	if (typeof err === 'object' && err !== null && 'isAxiosError' in err) {
		...
		if (isTokenExpiredMessage) {
			...
			setTimeout(() => {
				...
				alert('You have been logged out as the server indicated your access token has expired.');
			}, 100);
		}
	}
...
export type ResponseValues<Result, RequestData, ErrorResult = unknown> = {
	data?: Result;
	loading: boolean;
...
>(route: Route, args: {
	...
	if (typeof window === 'undefined') {
		return [{
			loading: true,
		}, async () => Promise.reject(new Error('Cannot fetch data outside a browser environment'))];
	}
...
export const asResponseValues = <TResponse, TBody, TError>(item: TResponse | undefined, inheritFrom: ResponseValues<unknown, TBody, TError>): ResponseValues<TResponse, TBody, TError> => ({
	...
	response: inheritFrom.response === undefined || item === undefined || item === null
		? undefined
		: {
			data: item,
