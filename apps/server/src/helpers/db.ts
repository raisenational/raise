...
import {
	DynamoDBDocumentClient, GetCommand, PutCommand, QueryCommand, ScanCommand, TransactWriteCommand, type TransactWriteCommandInput, type TransactWriteCommandOutput, UpdateCommand,
...
import {type JSONSchema, type AuditLog} from '../schemas';
import {auditLogTable, type DBAttributeValue, type Table} from './tables';
...
const normalizeGroup = (groupDefinition: string | string[] | {groupsWithAccess: string[]}): string[] => {
	...
	if (Array.isArray(groupDefinition)) {
		return groupDefinition;
	}
...
export const assertHasGroupForProperties = <B extends object>(event: {auth: {payload: {groups: string[]}}; body: B}, properties: (keyof B)[], ...groupDefinitions: (string | string[] | {groupsWithAccess: string[]})[]): void => {
	...
	if (!overlap(event.auth.payload.groups, groups)) {
		properties.forEach((p) => {
			if (p in event.body) {
				throw new createHttpError.Forbidden(`To edit ${String(p)} ${formatGroupError(groups, event.auth.payload.groups)}`);
			}
		});
	}
};
...
export const checkPrevious = <I extends Record<string, NativeAttributeValue>>(item: I): [Partial<I>, string, Record<string, NativeAttributeValue>, Record<string, NativeAttributeValue>] => {
	...
	return [
		...
		entries.reduce<Record<string, NativeAttributeValue>>((acc, [k, v]) => {
			acc[`:previous${k}`] = v;
			return acc;
		}, {}),
		entries.reduce<Record<string, NativeAttributeValue>>((acc, [k]) => {
			acc[`#${k}`] = k;
			return acc;
		}, {}),
	];
...
>(table: Table<Pa, Pr, S, K, E>): Promise<S[]> => {
	...
	while (true) {
		...
		calls += 1;
		if (calls > 10) {
			throw new createHttpError.InternalServerError(`Scan of ${table.entityName} exceeded call limit`);
		}
	}
...
>(table: Table<Pa, Pr, S, K, E>, key: Record<Pa, string>): Promise<S[]> => {
	...
	const result = await dbClient.send(new QueryCommand({
		...
		ExpressionAttributeValues: entries.reduce<Record<string, NativeAttributeValue>>((acc, [k, v]) => {
			acc[`:${k}`] = v;
			return acc;
		}, {}),
		ExpressionAttributeNames: entries.reduce<Record<string, NativeAttributeValue>>((acc, [k]) => {
			acc[`#${k}`] = k;
			return acc;
		}, {}),
...
>(table: Table<Pa, Pr, S, K, E>, key: K): Promise<S> => {
	...
	if (!result.Item) {
		throw new createHttpError.NotFound(`${table.entityName} not found`);
	}
...
export const insertAudit = async (auditDefinition: AuditDefinition): Promise<void> => {
	const id = ulid();
	try {
		if (auditContext.value === undefined) {
			throw new Error('auditContext is undefined');
		}
...
export const insert = async <
	...
	K extends Record<Pa | Pr, string>,
	E extends {[_K in keyof S]?: _K extends keyof K ? never : S[_K]},
>(table: Table<Pa, Pr, S, K, E>, data: S, extraConditionExpression?: string, extraAttributeValues?: Record<string, NativeAttributeValue>, extraAttributeNames?: Record<string, NativeAttributeValue>): Promise<S> => {
...
export const insertT = <
	...
	K extends Record<Pa | Pr, string>,
	E extends {[_K in keyof S]?: _K extends keyof K ? never : S[_K]},
>(table: Table<Pa, Pr, S, K, E>, data: S, extraConditionExpression?: string, extraAttributeValues?: Record<string, NativeAttributeValue>, extraAttributeNames?: Record<string, NativeAttributeValue>): {tDef: AWSTransactionDefinition; auditDef: AuditDefinition} => {
...
export const plusT = <
	...
	K extends Record<Pa | Pr, string>,
	E extends {[_K in keyof S]?: _K extends keyof K ? never : S[_K]},
>(table: Table<Pa, Pr, S, K, E>, key: K, data: E, extraConditionExpression?: string, extraAttributeValues?: Record<string, NativeAttributeValue>, extraAttributeNames?: Record<string, NativeAttributeValue>): {tDef: AWSTransactionDefinition; auditDef: AuditDefinition} => {
...
export const inTransaction = async (args: {tDef: AWSTransactionDefinition; auditDef: AuditDefinition}[]): Promise<TransactWriteCommandOutput> => {
	...
	await Promise.all(args.map(async (a) => insertAudit(a.auditDef)));
	return result;
};
...
>(table: Table<Pa, Pr, S, K, E>, key: K, listProperty: P, newItem: I, extraConditionExpression?: string, extraAttributeValues?: Record<string, NativeAttributeValue>, extraAttributeNames?: Record<string, NativeAttributeValue>): Promise<S> => {
...
export const update = async <
	...
	K extends Record<Pa | Pr, string>,
	E extends {[_K in keyof S]?: _K extends keyof K ? never : S[_K]},
>(table: Table<Pa, Pr, S, K, E>, key: K, edits: E, extraConditionExpression?: string, extraAttributeValues?: Record<string, NativeAttributeValue>, extraAttributeNames?: Record<string, NativeAttributeValue>): Promise<S> => {
