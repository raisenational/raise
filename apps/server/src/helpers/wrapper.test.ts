...
import {type AuditContext, auditContext} from './auditContext';
...
describe.each([
	...
	test('rejects an invalid auth header', async () => {
		...
		expect(response.statusCode).toBe(401);
		expect(JSON.parse(response.body as string).message).toContain('Format should be "Authorization: Bearer [token]"');
	});
	...
	test('rejects an auth token with an invalid payload', async () => {
		const response = await call(handler, {auth: {groups: [1]} as unknown as never, rawResponse: true})({greetee: 'world'});
...
describe('error handling', () => {
	describe.each([
		...
		test.each([
			...
			const handler = middyfy(null, null, false, async () => {
				throw err;
			});
			...
			expect(JSON.parse(response.body as string)).toEqual({message: expectedMessage});
		});
	});
});
...
describe('attaches audit context', () => {
	...
	const handler = middyfy(null, auditContextSchema, false, async () => {
		if (!auditContext.value) {
			throw new createHttpError.InternalServerError('Not in an auditContext');
		}
