...
import {
	insert, scan, get, query, insertAudit, update, inTransaction, updateT, plusT, insertT, type AuditDefinition, assertMatchesSchema, assertHasGroup, assertHasGroupForProperties, checkPrevious,
} from './db';
import {
	fundraiserTable, donationTable, auditLogTable, tables, type Table,
} from './tables';
...
describe.each([
	...
	test.each([
		...
		const e = expect(() => {
			assertHasGroup({auth: {payload: {groups: eventGroups}}}, checkGroups);
		});
		if (!notThrow) {
			e.toThrow();
		} else {
...
test('assertHasGroup handles multiple arguments', () => {
	expect(() => {
		assertHasGroup({auth: {payload: {groups: ['A']}}}, 'A', 'B');
	}).not.toThrow();
	expect(() => {
		assertHasGroup({auth: {payload: {groups: ['B']}}}, 'A', 'B');
	}).not.toThrow();
	expect(() => {
		assertHasGroup({auth: {payload: {groups: ['C']}}}, 'A', 'B');
	}).toThrow();
});
...
describe.each([
	...
	test.each([
		...
		const e = expect(() => {
			assertHasGroupForProperties({auth: {payload: {groups: eventGroups}}, body: {[eventProperty]: null}}, propertiesToCheck, checkGroups);
		});
		if (!notThrow) {
			e.toThrow();
		} else {
