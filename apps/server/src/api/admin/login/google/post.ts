...
export const main = middyfy($GoogleLoginRequest, $LoginResponse, false, async (event) => {
	if (!env.GOOGLE_LOGIN_ENABLED) {
		throw new createHttpError.Unauthorized('Google login is not enabled');
	}
	...
	if (!tokenPayload) {
		throw new createHttpError.Unauthorized('idToken: missing payload');
	}

	if (!tokenPayload.email) {
		throw new createHttpError.Unauthorized('idToken: missing email');
	}

	if (!tokenPayload.email_verified) {
		throw new createHttpError.Unauthorized('idToken: email not verified');
	}
