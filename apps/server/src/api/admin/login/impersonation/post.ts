...
export const main = middyfy($ImpersonationLoginRequest, $LoginResponse, false, async (event) => {
	if (!env.IMPERSONATION_LOGIN_ENABLED) {
		throw new createHttpError.Unauthorized('Impersonation login is not enabled');
	}

	// Prod should have IMPERSONATION_LOGIN_ENABLED disabled, but just as an extra check
	if (env.STAGE === 'prod') {
		throw new createHttpError.Unauthorized('Impersonation login should not be enabled in prod');
	}
