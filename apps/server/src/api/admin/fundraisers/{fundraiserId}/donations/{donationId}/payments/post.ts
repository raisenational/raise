...
export const main = middyfy($PaymentCreation, $Ulid, true, async (event) => {
	...
	if (donationAmount + donation.donationAmount < 0) {
		throw new createHttpError.BadRequest('This payment would cause the donation to have a negative donation amount');
	}

	if (contributionAmount + donation.contributionAmount < 0) {
		throw new createHttpError.BadRequest('This payment would cause the donation to have a negative contribution amount');
	}

	if (matchFundingAdded + donation.matchFundingAmount < 0) {
		throw new createHttpError.BadRequest('This payment would cause the donation to have a negative match funding amount');
	}
