...
export const main = middyfy($PublicDonationRequest, $PublicPaymentIntentResponse, false, async (event) => {
	...
	if (event.body.giftAid) {
		if (!event.body.addressLine1) {
			throw new createHttpError.BadRequest('Gift-aided donation must provide address line 1');
		}

		if (!event.body.addressPostcode) {
			throw new createHttpError.BadRequest('Gift-aided donation must provide address postcode');
		}

		if (!event.body.addressCountry) {
			throw new createHttpError.BadRequest('Gift-aided donation must provide address country');
		}
	}
	...
	if (!stripeClientSecret) {
		throw new Error('Failed to create Stripe client secret');
	}
	...
	await Promise.all([
		...
		...paymentSchedule.future.map(async (p) => (insert(paymentTable, {
			id: ulid(),
			donationId,
			fundraiserId: event.pathParameters.fundraiserId,
			at: p.at,
