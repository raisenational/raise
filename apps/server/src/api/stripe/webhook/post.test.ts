...
describe('signature validation', () => {
	...
	test('rejects request with invalid stripe signature', async () => {
		const req = makeStripeWebhookRequest();
		webhookConstructEvent.mockImplementationOnce(() => {
			throw new Error();
		});
...
test('can make first recurring donation without matchFundingPerDonationLimit', async () => {
	...
	await Promise.all([insert(fundraiserTable, fundraiser), insert(donationTable, donation), Promise.all(payments.map(async (p) => insert(paymentTable, p)))]);
...
test('handles parallel webhook calls for same one-off payment', async () => {
	...
	const responses = await Promise.all(new Array(25).fill(0).map(async () => call(main, {rawResponse: true, auth: false, headers: {'stripe-signature': 'valid-signature'}})(req)));
...
test('handles parallel webhook calls for same first recurring payment', async () => {
	...
	customersCreate.mock.calls.forEach((c) => {
		expect(c).toEqual([{
			name: donation.donorName,
			email: donation.donorEmail,
			metadata: {
