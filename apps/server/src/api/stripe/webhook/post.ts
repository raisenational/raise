...
export const main = middyfy($StripeWebhookRequest, null, false, async (event) => {
	...
	if (!signature) {
		throw new createHttpError.Unauthorized('Missing Stripe-Signature header');
	}
	...
	if (!auditContext.value) {
		throw new createHttpError.InternalServerError('Not in an audit context');
	}
	...
	const matchFundingAdded = payment.matchFundingAmount === null
		? calcMatchFunding({
