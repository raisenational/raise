...
import {type JSONSchema} from '../src/schemas';
import {type ExternalHandler} from '../src/helpers/types';
...
const verifyIsHandler = ({filePath, exports}: {filePath: string; exports: {main: ExternalHandler<any, any, any>}}) => {
	if (typeof exports !== 'object') {
		throw new Error(`Expected handler export an object, but didn't in ${filePath}`);
	}

	if (typeof exports.main !== 'function') {
		throw new Error(`Expected handler to have a named export 'main', but didn't in ${filePath}`);
	}

	if (typeof exports.main.requestSchema !== 'object') {
		throw new Error(`Expected handler main function to have an requestSchema object property, but didn't in ${filePath}`);
	}

	if (typeof exports.main.responseSchema !== 'object') {
		throw new Error(`Expected handler main function to have an responseSchema object property, but didn't in ${filePath}`);
	}

	if (typeof exports.main.requiresAuth !== 'boolean') {
		throw new Error(`Expected handler main function to have an requiresAuth boolean property, but didn't in ${filePath}`);
	}
};
