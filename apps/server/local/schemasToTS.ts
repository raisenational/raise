...
const main = async () => {
	...
	const deepClone = <T>(item: T): T => {
		if (Array.isArray(item)) {
			return item.slice().map(deepClone) as unknown as T;
		}
	...
	const sourceWithDuplicates = (await Promise.all(Object.entries(schemas).map(async ([k, v]) => compile(deepClone(v as JSONSchema4), k.replace(/^\$/, ''), {bannerComment: ''})))).join('\n');
	...
	const source = sourceWithDuplicates.split(/^export /m).map((d) => {
		...
		const name = (/(?:type|interface) (?<name>\w+) [={]/.exec(d))?.groups?.name;
		if (!name) {
			throw new Error(`Failed to determine name for code: ${d}`);
		}
